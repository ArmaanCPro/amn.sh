...
(Or at least, where the web wasn't mean't to be first-class...)

Moreover, there truly do come times where a very high-performance backend is necessary. It doesn't necessarily have to be for a website, though that is a commen use-case.
If you want performance, it just isn't feasible with Javascript or other high-level languages. For such an endevour, I would honestly recommend using a language that has more support for HTTP in it's standard library.
...
If you still want to use C++ for a production-grade application, checkout libraries like [Drogon](https://github.com/drogonframework/drogon) or [Boost.Beast](https://github.com/boostorg/beast).
...
And why over libuv? For no particular reason other than I was already familiar with Asio and wasn't with libuv. Both are great libraries and I would suggest you look at libuv for a wider perspective afterwards.
...
That just makes life a little easier for us, but there's not loss of learning. In order to understand HTTP/2, you need to understand HTTP/1.1.
...
HTTP Requests have a few main components:
1. Method
2. Path
3. Version
4. Headers
...
```
In this example, the client requests a GET at the path "/", which is basically asking the server for the index.html, the root html file that the browser should display. The server responds with pure HTML in the body.
The headers just provide more information about the request/response, which is _sometimes_ necessary, like Content-Type and Content-Length if we have a body.
...
```
Neat, right? No? Oh... you wondering where the HTMX is?
Wait, wait, I see it!
...
There we go. Now, we inject HTMX via a CDN, and we hook up the button to some htmx events. We're basically saying that when the button is clicked, sent a GET request to the path '/time' and swap the innerHTML of the element with id time.
That element is our div. The innerHTML of the div is just the text inside of it.

Very simple, I know, but enough to get us started!

#### Onto C++

Now, make sure you're C++ build system is linking to Boost.Asio and is set to C++23 (you may be able to get by with older versions, but I haven't tested them). Again, if you want more info on the build system, see the [Github repo I linked earlier](https://github.com/ArmaanCPro/aychTTP/tree/5cd9ba7e4c2d037f03f3588880674b90944a3df3).
...
Parsing HTTP, for our case so far, is suprisingly easy. We don't need to worry about headers or a body for our purposes today.
...
If you want to see your website, just run the server and open `http://localhost:3000` in your browser.
